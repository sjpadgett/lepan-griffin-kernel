 ...
 * adapted for lepan griffin display sjpadgett 9-4-2012
 * 
...
#define GRIFFIN_LCD_PIXCLOCK	54000		/* 51,2MHz */

static struct omap_video_timings griffin_panel_timings = {
		.x_res		= 1024,
		.y_res		= 768,

		.pixel_clock	= GRIFFIN_LCD_PIXCLOCK,	/* Mhz */
// 60hz
		.hsw	= 32,		/* horizontal sync pulse width */
...
{
	*width = 196;			/* display width in mm  */
	*height = 147;			/* display height in mm */
}

static int panel_probe(struct omap_dss_device *dssdev)
{
    dssdev->panel.config = OMAP_DSS_LCD_TFT;
	dssdev->panel.timings = griffin_panel_timings;
	printk( "**** panel_probe\n");
	return 0;
}

static struct omap_dss_driver griffin_panel = {
...
{
	printk( "griffin lcd panel init\n");

	omap_dss_register_driver(&griffin_panel);
	return 0;
}

static void __exit panel_drv_exit(void)
{
	omap_dss_unregister_driver(&griffin_panel);
}
